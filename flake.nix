rec {
  ...
  outputs =
    args @ { capacitor
    , ...
    }: capacitor args ({ self, inputs, systems, lib,... }: {
        config.systems = [ "x86_64-linux" "x86_64-darwin" "aarch64-linux" "aarch64-darwin"];
        config.plugins = [];
        passthru.lib = inputs.nixpkgs-unstable.lib;
        passthru = {
          legacyPackages = lib.genAttrs self.__reflect.finalFlake.config.systems (system: let
